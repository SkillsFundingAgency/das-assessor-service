...
{
    [TestFixture]
    public class When_called_for_organisation
    {
        ...
        [TestCase(2017, 12, 01)]
        [TestCase(2016, 12, 01)]
        public async Task Then_date_plus_six_months_is_returned(int year, int month, int day)
        {
            // Arrange
            ...
            // Act
            var result = await _sut.Handle(new GetEarliestWithdrawalDateRequest(Guid.NewGuid(), null), new CancellationToken());
