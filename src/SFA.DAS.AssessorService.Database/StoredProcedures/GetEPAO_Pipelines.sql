...
(
	...
	FROM (
	SELECT StdCode, EOMONTH(DATEADD(month, std.Duration, LearnStartDate)) AS EstimateDate , Title, UkPrn
	FROM (
	-- The most recent (as far as we can tell) record from ilr
	SELECT * FROM (
	SELECT Uln, StdCode,UkPrn, FamilyName, EPAORGID, completionStatus, row_number() OVER (PARTITION BY Uln, StdCode ORDER BY source DESC, learnstartdate desc) rownumber, 
	[LearnStartDate]
	FROM ilrs
