        ...
        public async Task<IEnumerable<OrganisationStandardSummary>> GetOrganisationStandardByOrganisationId(string organisationId)
        {
            ...
            var query = 
                @"SELECT 
                  ...
                  SELECT 
	                osv.StandardUId, os.StandardCode as LarsCode, s.Title, s.Level, s.IFateReferenceNumber, s.Version, s.VersionMajor, s.VersionMinor, osv.EffectiveFrom, osv.EffectiveTo, osv.DateVersionApproved, osv.Status
                  FROM 
