    ...
    public class SubmitApplicationSequenceHandler : IRequestHandler<SubmitApplicationSequenceRequest, bool>
    {
        ...
        private async Task NotifyContact(Domain.Entities.Contact contactToNotify, ApplyData applyData, int sequenceNo, CancellationToken cancellationToken)
        {
            ...
            if (sequenceNo == ApplyConst.ORGANISATION_SEQUENCE_NO)
            {
                ...
                await _mediator.Send(new SendEmailRequest(email, emailTemplateAlert, new { contactname, reference }), cancellationToken);
            }
            else if (sequenceNo == ApplyConst.STANDARD_SEQUENCE_NO)
            {
            ...
            {
                // TO DO: Send email to notify that the application has been smbmitted
            }
            else if (sequenceNo == ApplyConst.STANDARD_WITHDRAWAL_SEQUENCE_NO)
            {
                // TO DO: Send email to notify that the application has been submitted
            }
        }
    }
}
