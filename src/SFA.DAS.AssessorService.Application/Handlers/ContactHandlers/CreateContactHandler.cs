        ...
        public async Task<ContactBoolResponse> Handle(CreateContactRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (existingContact == null)
                {
                    ...
                    {
                        await _contactRepository.UpdateSignInId(contactResponse.Id,  request.GovIdentifier);
                    }
                }
                else
                {
                    if (!string.IsNullOrEmpty(request.GovIdentifier))
                    {
                        await _contactRepository.UpdateSignInId(existingContact.Id, request.GovIdentifier);
                    }
                }
            }
            catch(Exception ex)
