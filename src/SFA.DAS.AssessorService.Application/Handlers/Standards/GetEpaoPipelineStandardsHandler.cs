...
namespace SFA.DAS.AssessorService.Application.Handlers.Standards
{
    public class GetEpaoPipelineStandardsHandler : IRequestHandler<EpaoPipelineStandardsRequest, PaginatedList<EpaoPipelineStandardsResponse>>
    {
        ...
        public async Task<PaginatedList<EpaoPipelineStandardsResponse>> Handle(EpaoPipelineStandardsRequest request, CancellationToken cancellationToken)
        {
            ...
            var result = await _standardRepository.GetEpaoPipelineStandards(request.EpaoId, request.StandardFilterId, 
                request.ProviderFilterId, request.EPADateFilterId, request.OrderBy, request.OrderDirection, 
                request.PageSize, request.PageIndex, _config.PipelineCutoff);

            var epaoPipelineStandardsResult = result.PageOfResults.Select(o =>
            ...
            return new PaginatedList<EpaoPipelineStandardsResponse>(epaoPipelineStandardsResult, result.TotalCount, request.PageIndex ?? 1, request.PageSize);
        }
    }
}
